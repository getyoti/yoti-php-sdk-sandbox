...
class Receipt
{
    ...
    private function decryptAttribute(string $attributeName, PemFile $pemFile): ?string
    {
        ...
        //dump("ENCDATA", $encryptedData);
        //dump("wrap:", $this->getWrappedReceiptKey());

        //$v2DecData = EncryptedData::DecryptAttributeList($this->getWrappedReceiptKey(),  $encryptedData,$pemFile);
        //$key = PublicKeyLoader::load(file_get_contents('./keys/Prod.pem'), $password = false);
        //$vvv = CryptoEngine::decryptAttributeList($this->getWrappedReceiptKey(), $encryptedData, $pemFile);
        //dump($key);
        //dump($vvv);
